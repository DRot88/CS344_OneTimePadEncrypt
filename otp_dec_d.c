...
char decryptedMsg[BUF_SIZE];

//function declaration
char* decryptMsg(char* key, char* cypherText);
...
char* decryptMsg(char* key, char* cypherText) {
 ...
 for (i = 0; i < lenCypherMsg; i++) {
   for (j = 0; j < 27; j++) {
     ...
     decryptedMsg[i] = alphabet[decryptedValue]; // get character equivalent of encrypted value
   }
 }
 decryptedMsg[i] = '\0'; // set null terminator
    ...
    switch(spawnPid) {
      ...
      case 0: { // child case
       ...
       //   // send acknowledgement back to client
          ...
          char* decryptedMsgToSend;
          decryptedMsgToSend = decryptMsg(keyBuffer, buffer);

          // send encryptedMessage back to client
          charsRead = send(establishedConnectionFD, decryptedMsg, strlen(decryptedMsg), 0); // Send success back
          if (charsRead < 0) {
            error("ERROR writing to socket");
          }
        // }
